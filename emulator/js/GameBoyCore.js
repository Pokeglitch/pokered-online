 ...
 *
 */
function GameBoyCore(canvas, ROMImage, opts, addon) {
  if (!(this instanceof GameBoyCore)) return new GameBoyCore(canvas, ROMImage, opts, addon);

  if( addon ) this.addon = new Breakpoint( addon, this );
  ...
  this.intializeWhiteNoise();  //Initialize the white noise cache tables ahead of time:
  ...
  this.stopEmulator = 1;
  this.loop = setInterval(this.run.bind(this), this.opts.interval);
...
GameBoyCore.prototype.ROMLoad = function () {
  ...
  //Initialize the breakpoint byte so the game knows we are using this emulator
  if( this.addon )	this.addon.initBreakpointByte();
...
GameBoyCore.prototype.executeIteration = function () {
  ...
  while (this.stopEmulator == 0) {
	...
	//Check for breakpoints if they exist
	if( this.addon ) this.addon.checkBreakpoint();
