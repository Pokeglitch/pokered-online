...
GameBoyCore.prototype.start = function () {
	...
	this.recomputeDimension();		//Load the screen
}
...
GameBoyCore.prototype.executeIteration = function () {
	...
	while (this.stopEmulator == 0) {
		...
		for(var b in breakpoints){
			...
			//if we've reached the breakpoint, then run the associated callback function
			if( breakpoints[b].count == this.programCounter && (breakpoints[b].bank == this.ROMBank1offs || breakpoints[b].bank == 0) ){
				breakpoints[b].callback();
			}
